...
2. **이름/이미지 분리**: 특정 이미지 에셋(예: '다혈질청년남')을 사용하면서 올바른 이름(예: '엽문')을 표시하려면 다음 형식을 사용하십시오.
   - 형식: \`<대사>표시이름(에셋키)_감정: ...\`
   - 예시: \`<대사>엽문(다혈질청년남)_기쁨1: 어이!\` (이미지: 다혈질청년남, 이름: 엽문)
...
7. **[배경 전환]**: 장소가 바뀌거나 분위기가 전환될 때, 반드시 아래 태그를 사용하십시오.
   - **형식**: \`<배경>배경키워드\`
   - **예시**: \`<배경>객잔_1층\` / \`<배경>강호_숲길\`
   - **주의**: 배경 키워드는 위 [Available Backgrounds] 목록에 있는 것만 사용해야 합니다.
