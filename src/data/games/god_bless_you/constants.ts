...
4. **[성장의 카타르시스 (Growth Catharsis)]** (⭐ 신규):
   - **Mood가 'growth' 또는 'tension'일 때**: 코미디 비중을 **20% 이하**로 줄이십시오.
   - 수련/전투/깨달음 장면에서는 **진지한 묘사**가 우선입니다. 성공한 행동을 슬랩스틱으로 격하시키지 마십시오.
   - **PreLogic Score 8 이상(Success)**이면: 주인공의 행동이 **실제로 효과가 있어야** 합니다. 성공을 코미디로 뒤집지 마십시오.
   - **코미디 배분 기준**: 일상/교류 = 코미디 80% / 수련/전투/성장 = 코미디 20%.
   - 예: (수련 Scene) 운기조식 중 마기가 폭주하면, '웃기게 넘어짐'이 아니라 '이를 악물고 다시 시도'하는 장면이 개연성 있음.
`;
