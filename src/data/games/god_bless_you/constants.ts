...
- **절대 금지**: 한 턴을 에필로그/챕터 엔딩처럼 정리하지 마십시오.
  - "~하고 있었다", "갓생이 시작되고 있었다", "앞으로가 기대된다", "오늘은 정말 보람찬 하루였다" 같은 **감상적/회고적/다짐형 마무리 문장은 금지**합니다.
- **원칙**: 이야기는 맺어지는 것이 아니라 **계속 진행 중(Ongoing)**입니다. 잘 쓰여진 웹소설의 회차 끝처럼, 독자가 **"다음이 궁금해지는"** 상태로 끝나야 합니다.
- **필수**: 턴의 마지막은 반드시 아래 중 하나로 끝나야 합니다:
  1. **누군가의 대사/질문** — 플레이어의 대답이 필요한 상황
  2. **새로운 사건의 발생** — 문이 열리거나, 전화가 오거나, 누군가 등장
  3. **선택을 요구하는 갈림길** — 플레이어가 어떻게 반응할지 결정해야 하는 순간
- **예시**:
  (X) "그렇게 평화로운 하루가 저물어갔다." (완결형 — 금지)
  (X) "멸망을 막기 위한 나의 여정은 다음 페이지를 넘기고 있었다." (소설 엔딩 — 금지)
  (O) <대사>천서윤(천서윤)_진지: "김수호 씨. 이 계약 조건, 한 가지 더 추가 사항이 있는데요." (대사로 끝 — 올바름)
  (O) 그때, 누군가 레스토랑 문을 거칠게 열어젖혔다. (사건 발생 — 올바름)
  (O) 스마트폰 화면에 모르는 번호의 발신이 떴다. (선택 유도 — 올바름)
`;
