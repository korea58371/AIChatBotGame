...
export class AgentRetriever {
    ...
    static async retrieveContext(
        ...
        // 2. 모드별 검색 (Mode-Specific Retrieval)
        if (routerOut.type === 'combat') {
            ...
            // 대상 캐릭터 프로필 가져오기 (대상이 존재할 경우)
            if (routerOut.target) {
                const targetData = this.findCharacter(state, routerOut.target);
                if (targetData) {
    ...
    private static findCharacter(state: any, targetName: string): any {
        ...
        for (const [id, char] of Object.entries(state.characterData)) {
            const c = char as any;
            if (c.name?.toLowerCase().includes(lowerTarget) || id.includes(lowerTarget)) {
                return c;
            }
        }
        return null;
