...
async function checkProfile(emailOrId: string) {
    console.log(`Checking profile for: ${emailOrId}`);

    // 1. Try to find user by ID or Email from Auth
    let userId = emailOrId;

    // simplistic UUID check
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(emailOrId);

    if (!isUuid) {
        // Assume email, list users (requires admin)
...
if (!target) {
    console.log("Usage: npx ts-node scripts/debug_user_profile.ts <user_id_or_email>");
} else {
    checkProfile(target);
}
